...
const InvoicePrint = ({ id }) => {
    const invoiceRef = (0, react_1.useRef)();
    const [invoiceData, setInvoiceData] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        const fetchInvoiceData = async () => {
            if (id) {
                const response = await (0, invoice_action_1.getInvoice)(Number(id));
                if (response.success) {
    ...
    return (<div className="container mx-auto p-4">
      {/* Invoice Layout */}
      <div ref={invoiceRef} className="p-8 bg-white border rounded shadow-md printable">
        ...
        <>
            ...
            <table className="w-full border-collapse table-main">
              ...
              <tbody>
                ...
                {invoiceData.invoicedetails?.map((item, index) => (<tr key={index}>
      ...
      {/* Print Button */}
      <button onClick={handlePrint} type="button" className="mt-4 bg-green-600 text-white px-4 py-2 rounded">
        Print Invoice
      </button>
    </div>);
};
