...
const RoomRates_1 = __importDefault(require("./RoomRates"));
const GuestInformationForm_1 = __importDefault(require("./GuestInformationForm"));
const rooms = [
    {
        name: 'Room Only Rate',
        price: 'USD 89.08',
...
function BookingPage() {
    const [expandedRoom, setExpandedRoom] = (0, react_1.useState)(null);
    const toggleRoomRates = (roomName) => {
    ...
    return (<div className="p-8">
      ...
      <div className="flex space-x-4 mb-8">
        ...
        <input type="date" className="border p-2"/>
        <input type="text" placeholder="Promotion Code" className="border p-2"/>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-6">
          ...
          {rooms.map((room) => (<div key={room.name} className="border mb-4 p-4 bg-gray-100">
              <div className="flex justify-between">
                <div>
                  <h3 className="font-semibold">{room.name}</h3>
                  <p>From: {room.price}</p>
                </div>
                <button className="bg-gray-800 text-white px-4 py-2" onClick={() => toggleRoomRates(room.name)}>
                  View Rates
                </button>
              </div>
              {expandedRoom === room.name && <RoomRates_1.default />}
            </div>))}
        </div>
      ...
      <div>
        <GuestInformationForm_1.default />
      </div>
    </div>);
}
