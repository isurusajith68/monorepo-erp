...
const FormSchema = zod_2.z.object({
    ...
    rno: zod_2.z.coerce.number().min(2, {
        message: '',
    }),
    name: zod_2.z.string().min(2, {
        message: '',
    }),
    pnumber: zod_2.z.coerce.number().min(2, {
        message: '',
    }),
    email: zod_2.z.string().min(2, {
        message: '',
    }),
    pmethod: zod_2.z.string().min(2, {
        message: '',
    }),
    amount: zod_2.z.string().min(2, {
        message: '',
    }),
    baccount: zod_2.z.string().min(2, {
        message: '',
    }),
...
function ReciptForm({ id }) {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const fetchAccounts = async () => {
            const response = await (0, bank_action_1.SelectAllBank)();
            if (response.success) {
        ...
        if (id) {
            //form.setValue("id",id);
            const getPro = async () => {
                const pro = await (0, recipt_action_1.getRecipts)(Number(id ?? -1)); //-1 id set as-1(no user)
    ...
    async function onSubmit(data) {
        ...
        try {
            ...
            if (id) {
                ...
                const result = await (0, recipt_action_1.updateRecipts)(dirtyValues, id.toString());
                ...
                const result = await (0, recipt_action_1.InsertRecipt)(data);
    ...
    const getPrevItem = async () => {
        const prevItem = await (0, recipt_action_1.getPrevMaterialItem)(id ?? 0);
    ...
    const getNextItem = async () => {
        const nextItem = await (0, recipt_action_1.getNextMaterialItem)(id ?? 0);
    ...
    return (<div>
      {!id && (<h2 className="text-2xl font-bold mb-4 mt-20 ml-5">New Reciept</h2>)}
      {id && (<h2 className="text-2xl font-bold mb-4 mt-20 ml-5">Update Reciept</h2>)}

      {id && (<div className="flex flex-col-3 items-center justify-center lg:ml-[35%]">
      ...
      <div className="lg:ml-[75%] lg:mt-[-3%]">
        <button_1.Button className=" mr-5 bg-green-600 lg:ml-[15%]" type="button" onClick={() => router.push('/reciepts')}>
          View List
        </button_1.Button>
      </div>
      ...
      <div className="p-8 rounded-lg ml-10 lg:w-[90%] md:w-[90%] sm:w-[80%]">
        <form_1.Form {...form}>
          <form className="grid lg:grid-cols-2 sm:grid-cols-1 md:grid-cols-1 gap-4 ml-10" onSubmit={form.handleSubmit(onSubmit)}>
            <form_1.FormField control={form.control} name="rno" render={({ field }) => (<form_1.FormItem>
                  <div className=" mt-4 justify-items-center">
                    <div className="mb-2 ">
                      <form_1.FormLabel>Recipt NO </form_1.FormLabel>
                    </div>
                    <div>
                      <form_1.FormControl>
            ...
            <form_1.FormField control={form.control} name="name" render={({ field }) => (<form_1.FormItem>
                  <div className="mt-4 justify-items-center">
                    ...
                    <div>
                      <form_1.FormControl>
                        <input_1.Input placeholder="" {...field} className="p-2 border-2 border-green-600  mr-4 lg:ml-[10%] lg:w-[80%] md:w-[88%]  sm:w-[82%]"/>
                      </form_1.FormControl>
                    </div>
                  </div>
                  <form_1.FormMessage />
            ...
            <form_1.FormField control={form.control} name="pnumber" render={({ field }) => (<form_1.FormItem>
                  <div className="mt-4 justify-items-center">
                    <div className="mb-2 ">
            ...
            <form_1.FormField control={form.control} name="pmethod" render={({ field }) => (<form_1.FormItem>
                  <div className="mt-4 justify-items-center">
                    <div className="mb-2">
                      ...
                      <form_1.FormControl>
                        <select_1.Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <select_1.SelectContent>
                            <select_1.SelectItem value="Bank">Bank</select_1.SelectItem>
            ...
            <form_1.FormField control={form.control} name="baccount" render={({ field }) => (<form_1.FormItem>
                  <div className="mt-4 justify-items-center">
                    <div className="mb-2">
                      <form_1.FormLabel>Bank Account</form_1.FormLabel>
                    </div>
                    <div>
                      <form_1.FormControl>
                        <select_1.Select onValueChange={field.onChange} value={field.value}>
                          <select_1.SelectTrigger className="p-2 border-2 border-green-600  mr-4  lg:w-[80%] md:w-[88%]  sm:w-[82%]">
                            <select_1.SelectValue placeholder="select account"/>
                          </select_1.SelectTrigger>
                          <select_1.SelectContent>
                            <select_1.SelectGroup>
                              <select_1.SelectLabel>Accounts</select_1.SelectLabel>
                              {accounts.map((accounts) => (<select_1.SelectItem key={accounts.id} value={`${accounts.accnumber}`}>
                                  {accounts.accnumber}
                                </select_1.SelectItem>))}
                            </select_1.SelectGroup>
                          </select_1.SelectContent>
            ...
            <form_1.FormField control={form.control} name="project" render={({ field }) => (<form_1.FormItem>
                  <div className="mt-4 justify-items-center">
                    ...
                    <div>
                      <form_1.FormControl>
                        <select_1.Select onValueChange={field.onChange} value={field.value}>
                          <select_1.SelectTrigger className="p-2 border-2 border-green-600  mr-4 lg:ml-[10%] lg:w-[80%] md:w-[88%]  sm:w-[82%]">
                            <select_1.SelectValue placeholder="select project"/>
                          </select_1.SelectTrigger>
                          <select_1.SelectContent>
                            <select_1.SelectGroup>
                              <select_1.SelectLabel>Projects</select_1.SelectLabel>
                              {projects.map((projects) => (<select_1.SelectItem key={projects.pid} value={`${projects.pname}`}>
                                  {projects.pname}
                                </select_1.SelectItem>))}
                            </select_1.SelectGroup>
                          </select_1.SelectContent>
            ...
            <div className="justify-items-end flex">
              {!id && (<button_1.Button type="submit" className="mt-4 lg:ml-[45%] md:ml-[6%] md:w-[50%] lg:w-[20%]  w-2/6 px-4 py-2 bg-green-600 text-white  hover:bg-green-700">
                  Submit
                </button_1.Button>)}
              {id && (<button_1.Button type="submit" className="mt-4 lg:ml-[45%] md:ml-[6%] md:w-[50%] lg:w-[20%]  w-2/6 px-4 py-2 bg-green-600 text-white  hover:bg-green-700">
                  Update
