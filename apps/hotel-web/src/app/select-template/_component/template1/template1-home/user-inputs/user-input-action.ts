...
export const fetchHotels = async () => {
  ...
  try {
    const result = await client.query(`SELECT * FROM hoteldetails ORDER BY id ASC`);
...
export const InsertHotelData = async ({
  ...
  try {
    ...
    const hotelQuery = 'INSERT INTO hoteldetails (data) VALUES ($1) RETURNING id';
    const hotelValues = [JSON.stringify({ data, aboutFormData, serviceFormData, roomsFormData, contactdata })];
    ...
    const imageBuffer = imageData ? Buffer.from(await imageData.get('0').arrayBuffer()) : null;
    ...
    if (serviceImageData && serviceImageData.entries) {
      for (const [key, value] of serviceImageData.entries()) {
        ...
        const serviceImageValues = [null, null, serviceImageBuffer, null, hotelId];  // image is null for about images
