...
function ServicePage({ data, image }) {
      ...
      <section className="py-16 bg-gray-100">
        <div className="container mx-auto text-center ml-5 mr-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mr-10">
            {combinedData.length > 0 ? (combinedData.map((item, index) => (<div key={index} className="bg-white rounded-lg shadow-lg p-6 hover:bg-yellow-500 hover:text-white transition-all duration-300">
                  <div className="flex justify-center mb-4">
                    {item.serviceimage ? (<img src={item.serviceimage} alt="Service" width={50} height={50} className="ml-2"/>) : (<p>No image available</p>)}
                  </div>
