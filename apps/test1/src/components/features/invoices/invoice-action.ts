...
export const insertInvoice = async (data: any) => {
  ...
  try {
    ...
    data.invoicedetails = data.invoicedetails.map((r: any) => {
...
export const getInvoice = async (id: Number): Promise<any> => {
  ...
  try {
    ...
    const detailRows = db
      .prepare(
        `SELECT invoiceid,itemdetails,quentity,tax,amount,price FROM  invoicedetails  WHERE invoiceid=?`,
      )
      .all(id)
...
export const getFirstMaterialItemId = async (): Promise<any> => {
  const db = new Database(process.env.DB_NAME)
  try {
    db.pragma('journal_mode = WAL')
...
export const getLastMaterialItemId = async (): Promise<any> => {
  const db = new Database(process.env.DB_NAME)
  try {
    db.pragma('journal_mode = WAL')
    let prevRow, prevIdSql
...
export const insertInvoiceReceipt = async (data: any) => {
  console.log('data', process.env.DB_NAME, data)
  const db = new Database(process.env.DB_NAME)
  try {
    db.pragma('journal_mode = WAL')
    ...
    const totalPaidAmount = db
      .prepare(
        `SELECT sum(amount) as sum FROM  invoicereceipt  WHERE invoiceid=?`,
      )
...
export const updateInvoiceReceipt = async (dirtyfields: any, id: string) => {
