...
const NextLink = react_1.default.lazy(() => Promise.resolve().then(() => require('next/link')));
/**
 * SideBar component
 *
 ...
 */
const SideBar = ({ sideBarItems, userInfo, sideBarHeight, sideBarWidth, handleLogout, use, activeItem, }) => {
    // alert(sideBarHeight)
    return (<div className={`sticky top-0 left-0 bg-white ${sideBarHeight ? `h-[${sideBarHeight}]` : 'h-[100vh]'} flex flex-col justify-between rounded-lg shadow-lg z-10 ${'left-0'} ${sideBarWidth ? '' : 'min-w-[20vw]'}`} style={{
            minWidth: '200px',
      ...
      <div className="px-2">
        <div className="flex flex-col relative">
          <div className="flex items-center p-4 gap-3 ">
            <img src={userInfo.profileImage} alt="profile" className="w-12 h-12 rounded-full border-green-500 border-2"/>
            <div style={{
            width: '200px',
            overflow: 'hidden',
          ...
          {sideBarItems?.map((item, index) => (<div key={index} className="flex flex-col px-4 mt-2 gap-4 border-t p-2 border-white">
              {use === 'next' && (<NextLink href={item.path} className={item.path === activeItem
              ...
              {use === 'react' && (<react_router_dom_1.NavLink to={item.path} className={({ isActive }) => isActive
                    ? 'p-2  flex gap-2 text-blue-600 hover:bg-blue-800 hover:rounded-lg border transition duration-300 border-white shadow-sm bg-blue-900 rounded-lg cursor-pointer  items-center'
                    : 'p-2  flex gap-2 hover:bg-blue-500 hover:rounded-lg border transition duration-300  border-white shadow-sm bg-blue-700 rounded-lg cursor-pointer  items-center'}>
                  {item.icon}
                  <span className="ml-2 text-white">{item.name}</span>
      ...
      <div className="flex flex-col  px-4 gap-4 border-t p-2 border-white mb-4">
        <button onClick={handleLogout} className="p-2  flex gap-2 text-blue-600 hover:bg-red-500 hover:rounded-lg border transition duration-300 border-white shadow-sm bg-red-700 rounded-lg cursor-pointer items-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="24" height="24" className="w-6 h-6 text-white">
