...
const CalendarScreen = () => {
    const [checkInDate, setCheckInDate] = (0, react_1.useState)(null);
    const [checkOutDate, setCheckOutDate] = (0, react_1.useState)(null);
    const [currentMonth, setCurrentMonth] = (0, react_1.useState)(0); // 0 -> October 2024
    ...
    return (<div className="p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <button className={`p-2 ${currentMonth === 0 ? 'opacity-50' : ''}`} onClick={() => handleMonthChange('prev')} disabled={currentMonth === 0}>
          ←
        </button>
        <h2 className="text-lg font-semibold">{months[currentMonth]}</h2>
        <button className={`p-2 ${currentMonth === months.length - 1 ? 'opacity-50' : ''}`} onClick={() => handleMonthChange('next')} disabled={currentMonth === months.length - 1}>
          →
        </button>
      </div>
      ...
      <div className="grid grid-cols-7 gap-4 text-center">
        ...
        {/* Dates */}
        {initialDates.map(({ date, price }) => (<div key={date} className={`p-4 border rounded-lg ${date === checkInDate
                ? 'bg-green-200'
                : date === checkOutDate
                    ? 'bg-blue-200'
                    : 'hover:bg-gray-100'}`} onClick={() => handleDateClick(date)}>
