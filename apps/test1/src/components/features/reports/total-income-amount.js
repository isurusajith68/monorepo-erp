...
const group_incom_action_1 = require("./group-incom-action");
...
const YourComponent = ({ SetSharedIncome }) => {
    ...
    const [personIncome, setPersonIncome] = (0, react_1.useState)(0);
    const [dueIncome, setDueIncome] = (0, react_1.useState)(0);
    const [companyIncome, setCompanyIncome] = (0, react_1.useState)(0);
    (0, react_1.useEffect)(() => {
        (0, invoice_action_1.getAllInvoices)().then((response) => {
            if (response.success) {
        ...
        (0, group_incom_action_1.getAllIncomePerson)().then((response) => {
            if (response.success) {
        ...
        (0, group_incom_action_1.getAllIncomeCompany)().then((response) => {
            if (response.success) {
    ...
    const calculateTotalPerson = (income) => {
        let personIncome = 0;
        income.forEach((item) => {
    ...
    const calculateTotalCompany = (income) => {
        let companyIncome = 0;
        income.forEach((item) => {
