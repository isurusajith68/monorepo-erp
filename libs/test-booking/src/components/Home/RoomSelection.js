...
const RoomSelection = () => {
    ...
    return (<div className="p-6">
      ...
      {rooms.map((room) => (<div>
          ...
          <div key={room.name} className="border-b py-4 grid grid-cols-3 gap-1 ">
            <div className="flex items-start">
              <img src={room.imageUrl} alt={room.name} className="w-48 h-32 object-cover"/>
            </div>
            <div className="ml-4">
            ...
            <div className="">
              {room.deals.map((deal) => (<label key={deal.name} className="mb-2 flex items-center justify-between cursor-pointer hover:bg-gray-100 p-2 rounded-lg" onClick={() => setSelectedDeal(deal.name)} // Handle deal selection
            >
                  <div className="flex items-center">
                    <input type="radio" name="deal" className="mr-2" checked={selectedDeal === deal.name} // Control radio button state
