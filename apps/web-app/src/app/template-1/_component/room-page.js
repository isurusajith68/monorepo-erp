      ...
      <section id="room" className="py-16 bg-gray-100">
        <div className="container mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 ml-5 mr-5">
            {combinedData.length > 0 ? (combinedData.map((item, index) => (<div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden">
                  <div>
                    {item.roomimage ? (<img src={item.roomimage} alt="Service" width={50} height={50} className="w-full h-64 object-cover"/>) : (<p>No image available</p>)}
                  </div>
                  <div className="p-6">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="flex items-center">
                        {[...Array(item.rating)].map((_, i) => (<i key={i} className="fa fa-star text-yellow-500"></i>))}
                      </div>
                    </div>
